...
Tunning mtry - the rf model could be furhter optimized.
...
Inspecting the training object of XgbTree, we see that the model is tested for 108 combinations of 7 tunning parameters. It is possible to optimize the parameters and a better result could be obtained:


```{r - XgbTree - best tunning, message=FALSE, echo=FALSE}
#xgbTree model - best tunning parameters
  ...
  y_hat_Value_Best_xgbTree <- predict(train_xgbTree_Best, test_set_scaled, type="raw")
  RMSE_xgbTree_Best <- RMSE(test_set_scaled$Value, y_hat_Value)
  WMAPE_xgbTree_Best <- WMAPE(test_set_scaled$Value, y_hat_Value)
  rmse_results <- bind_rows(rmse_results, data_frame(method="xgbTree best tune",
                                                       RMSE = RMSE_xgbTree_Best, WMAPE=WMAPE_xgbTree_Best))
```

```{r - XgbTree - best tunning - to print, message=FALSE}
  #xgbTree model - best tunning parameters
    RMSE_xgbTree_Best
    WMAPE_xgbTree_Best
```
...
```{r - Plot - Absolute error of prediction vs. Value for XgbTree , message=FALSE, echo=FALSE , fig.align='center'}
  test_set_scaled %>% mutate(y_hat = y_hat_Value_Best_xgbTree, absolute_error = abs(y_hat - Value), 
...
test_set_scaled %>% mutate(y_hat = y_hat_Value_Best_xgbTree, absolute_error = abs(y_hat - Value)) %>%  select(Value, absolute_error) %>% arrange(desc(Value)) %>% mutate(Value_Rank = seq(1, nrow(.), 1)) %>% arrange(desc(absolute_error)) %>% head(n=10)
```
